...
class ViewController: UIViewController {
    ...
    @IBAction func btnLampRemove(_ sender: UIButton) {
        ...
        let offAction = UIAlertAction(title: "아니요, 끕니다(off)", style: UIAlertAction.Style.default) { UIAlertAction in
            self.lampImg.image = self.imgOff
            self.isLampOn = false
        }
        let onAction = UIAlertAction(title: "아니요, 켭니다(on)", style: UIAlertAction.Style.default) { UIAlertAction in
            self.lampImg.image = self.imgOn
            self.isLampOn = true
        }
        let removeAction = UIAlertAction(title: "네, 제거합니다.", style: UIAlertAction.Style.default) { UIAlertAction in
